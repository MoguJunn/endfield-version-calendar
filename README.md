...
- Version 6「雪凇幽梦」采用冰蓝与淡紫配色；切换历史版本时恢复原配色，亮暗模式偏好保持不变。
...
Version 6「雪凇幽梦」依据官方版本日历与罗德岛蜜饼工坊的版本内容时间轴（Ver 26.08.29b）整理，活动定义位于 `lib/version-six.js`。战争回响、理智补给和影拓丰碑复用现有素材，其他新活动暂不配图；未公布的预热签到开始时间仍明确标注为待公布。

补充截图确认「影拓丰碑」新系列名称为「幽影刻形」，10 月 5 日 12:00 起常驻开放；同期「丰碑留名 · 刻影」奖励期至 10 月 19 日 04:00。

Version 5「向渊行」主要依据 [《明日方舟：终末地》官方发布的版本日历](https://www.bilibili.com/opus/1224859391493144580) 整理。

卡池正式 ID 替换旧手动 ID 时，日历根据类别、名称和开池日期唯一匹配后合并，保留主站图片。重构寻访、重构申领保留各自名称；常驻内容在下一版本分割线处结束绘制，实际仍为常驻开放。最新日历已明确的第六版日程优先于旧快照的占位时间。

项目早期也参考了 [罗德岛蜜饼工坊发布的「向渊行」非官方活动时间轴](https://www.bilibili.com/opus/1223774493520953353)，特别感谢原作者第一时间整理并公开分享。本项目在公开日历信息基础上实现实时状态、倒计时、筛选和详情交互。若网页内容与游戏内显示不一致，请以官方公告和游戏内实际时间为准。
...
Version 5 活动位于 `lib/calendar-core.js` 的 `rawEvents` 数组，Version 6 活动位于 `lib/version-six.js` 的 `versionSixEvents` 数组，页面与公开 API 共用这些定义。基础数据结构示例：
