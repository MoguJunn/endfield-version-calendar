import { versionSix } from "./version-six.js";

export const MAIN_SITE_STATS_API = "https://ef-gacha.mogujun.icu/api/stats";
export const OFFICIAL_VERSION_CALENDAR_SOURCE = "https://www.bilibili.com/opus/1224859391493144580";
...
export const rawEvents = Object.freeze([
  ...
  },
  {
    ...
    category: "arsenal", title: "「军列申领」", start: "2026-07-16T12:00:00+08:00", end: "2026-09-24T11:59:00+08:00",
    endLabel: null, calendarTiming: true, color: "#48c2d5", lane: 0,
    symbol: "轮", visual: "arsenal", description: "跨版本武库申领，按最新版本日历于 9 月 24 日 11:59 结束。",
  },
  {
  ...
  },
  {
    ...
    endLabel: "「冬猎」后第1个特许寻访结束时", calendarTiming: true, color: "#b54ad2", lane: 1,
    symbol: "瞳", visual: "arsenal", description: "跨版本持续开放。按最新日历，于「冬猎」后的第 1 个特许寻访结束时关闭，暂不采用数据库中的预估结束日期。",
  },
  {
...
export const fallbackVersion = Object.freeze({
  ...
  sourceMeta: { source: OFFICIAL_VERSION_CALENDAR_SOURCE, author: "明日方舟：终末地官方" },
});
...
export const latestFallbackVersion = versionSix;

export function mergeCalendarVersions(remoteVersions = []) {
  const byKey = new Map(remoteVersions.map((version) => [version.versionKey, version]));
  fallbackVersions.forEach((local) => {
    if (!byKey.has(local.versionKey)) byKey.set(local.versionKey, local);
  });
  return [...byKey.values()].map((version) => ({
    ...version,
    content: {
      ...version.content,
      ...(version.versionKey === versionSix.versionKey ? { activitiesComplete: true } : {}),
    },
...
export function normalizeEvents(sourceEvents, { timelineEnd = Number.POSITIVE_INFINITY } = {}) {
  ...
  const normalized = input.map((event) => {
    ...
    return { ...event, start, end, trackType: eventTrackType(event), startDate: new Date(start), endDate: end ? new Date(end) : null,
      displayEndDate: event.displayEnd ? new Date(event.displayEnd) : null };
  ...
  categories.filter((category) => !category.overlay).forEach((category) => {
    ...
    typeOrder.forEach((trackType) => {
      ...
      categoryEvents.filter((event) => event.trackType === trackType)
        ...
        .forEach((event) => {
          const eventEnd = event.displayEndDate?.getTime() ?? event.endDate?.getTime() ?? Number(timelineEnd);
...
export function poolKindOf(pool) {
  return /reconstruction|重构/u.test(`${pool.poolKind || ""} ${pool.visual || ""} ${pool.type || ""} ${pool.poolId || pool.id || ""} ${pool.title || ""}`)
...
export function formatDatabasePoolTitle(event, poolName) {
  ...
  if (poolKindOf(event) === "reconstruction") {
    return `「${name}」${event.category === "arsenal" ? "重构申领" : "重构寻访"}`;
  }
  return event.category === "operator" ? `「${name}」特许寻访` : `「${name}」`;
}

function poolIdentity(pool) {
  const start = Date.parse(pool.startsAt || pool.start || "");
  const name = pool.name || pool.databasePoolName || String(pool.title || "").match(/^「([^」]+)」/u)?.[1];
  if (!name || !Number.isFinite(start)) return null;
  const day = new Date(start + 8 * 3600000).toISOString().slice(0, 10);
  return `${pool.category || (isLegacyWeaponPool(pool) ? "arsenal" : "operator")}:${cleanDatabasePoolName(name)}:${day}`;
}

function isManualPool(pool) {
  return /manual/iu.test(pool.poolId || "");
}

// 仅在同类别、同名、同日且正式记录唯一时接续手动 ID，避免吞掉同名复刻池。
function canonicalPoolCatalog(pools) {
  const byId = new Map();
  pools.forEach((pool) => {
    if (pool?.poolId) byId.set(pool.poolId, { ...byId.get(pool.poolId), ...pool });
  });
  const groups = new Map();
  byId.forEach((pool) => {
    const key = poolIdentity(pool);
    if (key) groups.set(key, [...(groups.get(key) || []), pool]);
  });
  const replacements = new Map();
  groups.forEach((group) => {
    const official = group.filter((pool) => !isManualPool(pool));
    if (official.length !== 1) return;
    group.filter(isManualPool).forEach((pool) => replacements.set(pool.poolId, official[0].poolId));
  });
  return {
    pools: [...byId.values()].filter((pool) => !replacements.has(pool.poolId)),
    replacements,
  };
}
...
export function isLegacyWeaponPool(pool) {
  return /(?:arsenal|weapon|wepon|武器|申领)/iu.test(`${pool?.type || ""} ${pool?.id || pool?.poolId || ""} ${pool?.name || ""}`);
}
...
export function poolToEvent(pool, { baseUrl = MAIN_SITE_STATS_API } = {}) {
  const category = isLegacyWeaponPool(pool) ? "arsenal" : "operator";
  const name = cleanDatabasePoolName(pool.name);
  const schedule = curatedPoolSchedule(pool);
  const poolKind = schedule?.poolKind || poolKindOf(pool);
  return {
    id: `pool-${pool.poolId}`, poolId: pool.poolId, category, poolKind,
    title: formatDatabasePoolTitle({ category, poolKind }, name),
    start: schedule?.start || pool.startsAt, end: schedule ? schedule.end : pool.endsAt,
    endLabel: schedule?.endLabel || null,
    ...poolColorTheme(pool, category), symbol: category === "operator" ? "访" : "武",
    visual: poolKind === "reconstruction" ? "reconstruction" : category, image: resolveMainSiteAssetUrl(pool.backgroundUrl || pool.bannerUrl, baseUrl),
...
function curatedPoolSchedule(pool) {
  const identity = poolIdentity(pool);
  return [...rawEvents.filter((event) => event.calendarTiming), ...versionSix.content.events]
    .find((event) => event.poolId && identity && poolIdentity(event) === identity);
}

export function buildEventsForVersion(version, options = {}) {
  const localFallback = options.fallbackVersion || fallbackVersions.find((local) => local.versionKey === version?.versionKey);
  const localEvents = version?.versionKey === localFallback?.versionKey ? (options.rawEvents || localFallback.content.events) : [];
  ...
  const merged = remoteEvents.map((event) => {
    const localEvent = localById.get(event.id) || {};
    return {
      ...localEvent,
      ...event,
      ...(version.versionKey === versionSix.versionKey || localEvent.calendarTiming ? localEvent : {}),
      image: event.image || localEvent.image || null,
      backgroundCharacter: event.backgroundCharacter || localEvent.backgroundCharacter || null,
      backgroundType: event.backgroundType || localEvent.backgroundType || null,
    };
  });
  ...
  // 公开 API 回传的事件也携带正式 poolId，浏览器重新合并本地备份时同样使用它。
  const eventPools = remoteEvents.filter((event) => event.poolId && ["operator", "arsenal"].includes(event.category)).map((event) => ({
    poolId: event.poolId, name: String(event.title || "").match(/^「([^」]+)」/u)?.[1],
    type: event.category, poolKind: poolKindOf(event), startsAt: event.start, endsAt: event.end,
    backgroundUrl: event.image, backgroundCharacter: event.backgroundCharacter, backgroundType: event.backgroundType,
  }));
  const { pools, replacements } = canonicalPoolCatalog([...eventPools, ...(options.pools || version?.pools || [])]);
  const poolsById = new Map(pools.map((pool) => [pool.poolId, pool]));
  const represented = new Set();
  const enriched = merged.map((event) => {
    const boundId = bindings[event.id] || event.poolId || null;
    let poolId = replacements.get(boundId) || boundId;
    let pool = poolId ? poolsById.get(poolId) : null;
    if (!pool && ["operator", "arsenal"].includes(event.category)) {
      const identity = poolIdentity(event);
      const matches = pools.filter((candidate) => identity && poolIdentity(candidate) === identity);
      if (matches.length === 1) { pool = matches[0]; poolId = pool.poolId; }
    }
    if (poolId) represented.add(poolId);
    if (!pool) return { ...event, poolId, color: event.color || poolColorTheme({ poolId, name: event.title }, event.category).color };
    const curatedTiming = event.calendarTiming || (version.versionKey === versionSix.versionKey && localById.has(event.id));
    const schedule = curatedPoolSchedule(pool);
    const knownPoolEnd = Number.isFinite(Date.parse(pool.endsAt || ""));
    const end = schedule ? schedule.end : (curatedTiming ? event.end : (knownPoolEnd ? pool.endsAt : event.end));
    const poolKind = poolKindOf(event) === "reconstruction" ? "reconstruction" : poolKindOf(pool);
    return {
      ...event, poolId, poolKind, title: formatDatabasePoolTitle({ ...event, poolKind }, pool.name),
      color: event.color || poolColorTheme(pool, event.category).color,
      visual: poolKind === "reconstruction" ? "reconstruction" : event.visual,
      ...
      start: schedule?.start || (curatedTiming ? event.start : (pool.startsAt || event.start)), end,
      endLabel: end ? null : (schedule?.endLabel || event.endLabel),
      databasePoolName: pool.name,
    };
  });
  pools.forEach((pool) => { if (!represented.has(pool.poolId) && poolOverlapsVersion(pool, version)) enriched.push(poolToEvent(pool, options)); });
  const versionStart = Date.parse(version?.startsAt || "");
  const versionEnd = Date.parse(version?.endsAt || "");
  const unique = new Map();
  enriched.forEach((event) => {
    const key = event.poolId ? `pool:${event.poolId}` : event.id;
    const existing = unique.get(key);
    if (!existing || (String(existing.id).startsWith("pool-") && !String(event.id).startsWith("pool-"))) unique.set(key, event);
  });
  const bounded = [...unique.values()].map((event) => ({
    ...event,
    displayEnd: event.permanent && !event.end && Number.isFinite(versionEnd) ? version.endsAt : event.displayEnd || null,
  }));
  return normalizeEvents(bounded, { timelineEnd: versionEnd }).filter((event) => {
...
export function buildSeamlessEvents(versions, options = {}) {
  ...
  const ordered = [...(Array.isArray(versions) ? versions : [])].sort((a, b) => Date.parse(a.startsAt) - Date.parse(b.startsAt));
  const pools = canonicalPoolCatalog(ordered.flatMap((version) => version.pools || [])).pools;
  ordered.forEach((version, index) => {
    buildEventsForVersion(version, { ...options, ...(pools.length ? { pools } : {}) }).forEach((event) => {
      ...
      if (event.permanent && !event.end) candidate.displayEnd = ordered[index + 1]?.startsAt || version.endsAt || event.displayEnd || null;
